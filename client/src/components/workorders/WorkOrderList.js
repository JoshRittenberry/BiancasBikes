...
import { getIncompleteWorkOrders, updateWorkOrder, completeWorkOrder, deleteWorkOrder } from "../../managers/workOrderManager";
...
export default function WorkOrderList({ loggedInUser }) {
    ...
    const completeWorkOrderButton = (workOrder) => {
        completeWorkOrder(workOrder).then(() => {
            getIncompleteWorkOrders().then(setWorkOrders);
        })
    };

    const deleteWorkOrderButton = (workOrderId) => {
                    ...
                    {workOrders.map((wo) => (
                        <tr key={wo.id}>
                            ...
                            <td>
                                {wo.userProfile && (
                                    <Button
                                        onClick={() => completeWorkOrderButton(wo)}
                                        color="success"
                                    >
                                        Mark as Complete
                                    </Button>
